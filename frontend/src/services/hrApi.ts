...
export const employeeApi = {
  ...
  getEmployeeByNumber: async (employeeNumber: string): Promise<Employee> => {
    ...
    const { data } = await api.get<ApiResponse<Employee>>(
      `${API_BASE_URL}/employees/number/${employeeNumber}`
    )
    return data.data
  },
  ...
  getEmployeeByEmail: async (email: string): Promise<Employee> => {
    ...
    const { data } = await api.get<ApiResponse<Employee>>(
      `${API_BASE_URL}/employees/email/${email}`
    )
    return data.data
  },
  ...
  getEmployeesByCompany: async (
    ...
    const { data } = await api.get<ApiResponse<PageResponse<Employee>>>(
  ...
  getActiveEmployeesByCompany: async (
    ...
    const { data } = await api.get<Employee[]>(
      `${API_BASE_URL}/employees/active/company/${companyId}`
    )
    return data || []
  },
   ...
   * 입사일 범위로 직원 조회
   */
  getEmployeesByHireDateRange: async (
    startDate: string,
    endDate: string
  ): Promise<Employee[]> => {
    const { data } = await api.get<Employee[]>(
      `${API_BASE_URL}/employees/hire-date`,
      {
   ...
   * 생일인 직원 조회
   */
  getEmployeesByBirthday: async (
    month: number,
    day: number
  ): Promise<Employee[]> => {
    const { data } = await api.get<ApiResponse<Employee[]>>(
      `${API_BASE_URL}/employees/birthday`,
      {
        params: { month, day },
      }
    )
  ...
  getBirthdayEmployeesThisMonth: async (): Promise<Employee[]> => {
    const { data } = await api.get<ApiResponse<Employee[]>>(
      `${API_BASE_URL}/employees/birthday/this-month`
    )
    return data.data
  },
  ...
  terminateEmployee: async (
    ...
    await api.patch<ApiResponse<void>>(
      `${API_BASE_URL}/employees/${id}/terminate`,
      null,
      {
  ...
  reactivateEmployee: async (id: number): Promise<void> => {
    await api.patch<ApiResponse<void>>(
      `${API_BASE_URL}/employees/${id}/reactivate`
    )
  },
  ...
  checkEmployeeNumber: async (
    ...
    try {
      const { data } = await api.get<ApiResponse<boolean>>(
        `${API_BASE_URL}/employees/check/employee-number`,
        {
  ...
  checkEmail: async (email: string, excludeId?: number): Promise<boolean> => {
    ...
    try {
      const { data } = await api.get<ApiResponse<boolean>>(
        `${API_BASE_URL}/employees/check/email`,
        {
  ...
  getRecentEmployeesByCompany: async (
    ...
    try {
      const response = await api.get<ApiResponse<Employee[]>>(
        `${API_BASE_URL}/employees/recent/company/${companyId}`
      )
  ...
  getEmployeeCountByPosition: async (): Promise<StatisticsData[]> => {
    ...
    const { data } = await api.get<ApiResponse<[string, number][]>>(
      `${API_BASE_URL}/employees/statistics/position`
    )
  ...
  getEmployeeCountByDepartment: async (): Promise<StatisticsData[]> => {
    ...
    const { data } = await api.get<ApiResponse<[string, number][]>>(
      `${API_BASE_URL}/employees/statistics/department`
    )
    return data.data.map(([label, count]) => ({ label, count }))
  },
  ...
  getEmployeeCountByHireYear: async (): Promise<StatisticsData[]> => {
    ...
    const { data } = await api.get<ApiResponse<[number, number][]>>(
      `${API_BASE_URL}/employees/statistics/hire-year`
    )
  ...
  getEmployeeCountByAgeGroup: async (): Promise<StatisticsData[]> => {
    ...
    const { data } = await api.get<ApiResponse<[string, number][]>>(
      `${API_BASE_URL}/employees/statistics/age-group`
    )
    return data.data.map(([label, count]) => ({ label, count }))
  },
  ...
  getEmployeeCountByGender: async (): Promise<StatisticsData[]> => {
    ...
    const { data } = await api.get<ApiResponse<[string, number][]>>(
      `${API_BASE_URL}/employees/statistics/gender`
    )
  ...
  importFromExcel: async (
    ...
    const response = await api.post(
      `${API_BASE_URL}/employees/import/excel`,
      formData,
      {
  ...
  importFromCsv: async (
    ...
    const response = await api.post(
      `${API_BASE_URL}/employees/import/csv`,
      formData,
      {
        headers: { 'Content-Type': 'multipart/form-data' },
      }
...
export const positionApi = {
  ...
  getAllPositions: async (): Promise<Position[]> => {
    ...
    try {
      const response = await api.get<ApiResponse<PageResponse<Position>>>(
        '/positions',
        {
          params: { page: 0, size: 1000 },
        }
   ...
   * 회사별 활성 직급 목록 조회
   */
  getActivePositionsByCompany: async (
    companyId: number
  ): Promise<Position[]> => {
    const { data } = await api.get<ApiResponse<Position[]>>(
      `/positions/company/${companyId}/active`
    )
    return data.data
  },
   ...
   * 회사별 직급 코드 중복 확인
   */
  checkPositionCodeInCompany: async (
    companyId: number,
    positionCode: string,
    excludeId?: number
  ): Promise<boolean> => {
    const { data } = await api.get<ApiResponse<boolean>>(
      `/positions/check/position-code/company/${companyId}`,
      {
        params: { positionCode, excludeId },
      }
    )
